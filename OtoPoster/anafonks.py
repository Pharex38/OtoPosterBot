...
async def kanalmenu(update, context):
    ...
    if mesaj == "🔶 Yeni Kanal Ekle":
        ...
        if len(kudat['kanal']) > 34 and not user in vip_uyeler:
            await bot.send_message(chat, "<i>Üzgünüm en fazla 35 kanal kaydedebilirsiniz.</i>")
            return 
...
async def postmenu(update, context):
    ...
    if mesaj == "⏱ Post Zamanları":
        ...
        for kca in poudat['kanal']:
            admsl = await bot.get_chat_administrators(kca)
            if not eklenti in [r.user.id for r in (admsl)]:

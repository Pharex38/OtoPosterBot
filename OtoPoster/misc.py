...
def deep(u_kod, user):
    ...
    if int(u_kod) > 100:
        ...
        if kat == None:
            ...
            return True
    key = {"_id": user, "kanal": [], "sablon": "1", "kaynak": [str(u_kod)], "site": "1", "altapi": "None", "altsite": "None", "sira": "0", "ozel": False, "pcount": 0, "time": 0, "vakit": 0, "begeni": [], "pin": [], "eski": []}
    if kat == None:
...
def eklentiiletisim(update, context):
    ...
    if ileti.split("+")[0].isdigit():
        ...
        bot.edit_message_text(str(bot.get_chat(ileti[1]).title)+" "+ileti[2], ileti[0], imsgid)
        return
